    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-gray-50 flex-shrink-0">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-pink-600 to-pink-700 flex items-center justify-center shadow-sm">
            <Activity className="text-white" size={16} />
          </div>
          <h2 className="text-lg font-bold text-gray-900">Execution Monitor</h2>
        </div>
        
        {/* Connection Status */}
        <div className="flex items-center space-x-3">
          {sessionId && (
            <span className="text-xs text-gray-600 font-mono bg-gray-100 px-2 py-1 rounded-lg border border-gray-200">
              {sessionId.slice(0, 8)}
            </span>
          )}
      ...
      {/* Error Display */}
      {error && (
        <div className="p-5 bg-red-50 border-b border-red-200 flex-shrink-0">
            ...
            {sessionId ? (
              <div className="space-y-4">
                <div className="w-12 h-12 mx-auto rounded-xl bg-pink-100 flex items-center justify-center">
                  <Activity className="animate-pulse text-pink-600" size={24} />
                </div>
              <div className="space-y-2">
                  <p className="font-bold text-lg text-gray-900">Monitoring Execution</p>
                  <p className="text-sm text-gray-600">Waiting for workflow events...</p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
              ...
              <div className="space-y-2">
                  <p className="font-bold text-lg text-gray-900">Ready to Execute</p>
                  <p className="text-sm text-gray-600">Define your workflow and execute to see real-time logs</p>
                </div>
              </div>
            )}
          </div>
...
function LogEntry({ entry }: LogEntryProps) {
  ...
  return (
    <div className={`p-4 rounded-xl border ${levelBgColor} transition-all hover:bg-gray-50 group max-w-full shadow-sm`}>
      <div className="flex items-start space-x-4 min-w-0">
        {/* Timestamp */}
        <span className="text-xs text-gray-600 font-mono min-w-[70px] mt-1 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200">
          {new Date(entry.timestamp).toLocaleTimeString()}
        </span>
        
        {/* Message and content */}
        <div className="flex-1 min-w-0 overflow-hidden">
          <p className={`text-sm ${levelColor} break-words font-medium leading-relaxed overflow-wrap-anywhere`}>
            {entry.message}
          </p>
          ...
          {entry.structuredData?.type === 'reasoning' && (
            <div className="mt-3">
              <span className={`inline-flex items-center text-xs px-3 py-1 rounded-lg font-medium ${
                entry.structuredData.confidence === 'high' ? 'bg-green-50 text-green-700 border border-green-200' :
                entry.structuredData.confidence === 'medium' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' :
                'bg-red-50 text-red-700 border border-red-200'
              }`}>
                <div className={`w-1.5 h-1.5 rounded-full mr-2 ${
                  entry.structuredData.confidence === 'high' ? 'bg-green-500' :
                  entry.structuredData.confidence === 'medium' ? 'bg-yellow-500' :
                  'bg-red-500'
                }`}></div>
          ...
          {entry.structuredData && (
            <div className="flex items-center space-x-3 mt-3">
              ...
              <span className="text-xs text-gray-500 font-medium bg-gray-50 px-2 py-1 rounded-lg border border-gray-200">
